...
# def nonlinearity(x):
#     # swish
#     return (x * torch.sigmoid(x)).clamp_(max=10.0)
...
class Upsample3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.with_conv:
            ...
            # x = torch.nn.functional.pad(x, (0, 0, 0, 0, 2, 0), mode="replicate")
            # x = torch.cat([x[:, :, :1, :, :].repeat(1, 1, 2, 1, 1), x], dim=2)
            x = x.to(ori_dtype)
            x = self.conv(x)
            x = rearrange(x, "b (n c) t h w -> b c (t n) h w", n=self.up_time)
            # x = torch.cat(
            #     [
            #         x[:, :, : self.up_time, :, :].mean(dim=2, keepdims=True),
            #         x[:, :, self.up_time :, :, :],
            #     ],
            #     dim=2,
            # )
...
class CausalConv3d(torch.nn.Conv3d):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        # input = torch.cat(
        #     [input[:, :, :1, :, :].repeat(1, 1, 2 * self.custom_padding, 1, 1), input],
        #     dim=2,
        # )
...
class Decoder2(Decoder):
    def __init__(
        ...
        **ignorekwargs,
        ...
        make_conv_cls = self._make_conv()
        # z to block_in
        conv_3d_cls = CausalConv3d if self.causal else nn.Conv3d
        self.conv_in = conv_3d_cls(
            z_channels, block_in, kernel_size=3, stride=1, padding=1
        )
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                ...
                up.upsample = Upsample3D(
                    block_in, resamp_with_conv, up_time, causal=causal
                )
                curr_res = curr_res * 2
            self.up.insert(0, up)  # prepend to get consistent order
