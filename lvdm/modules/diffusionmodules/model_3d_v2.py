...
class Upsample3D(nn.Module):
    def __init__(self, in_channels, with_conv=True, up_time=False):
        super().__init__()
...
class Downsample3D(nn.Module):
    def __init__(self, in_channels, with_conv=True, down_time=False):
        super().__init__()
        self.with_conv = with_conv
        assert with_conv is True
        ...
        self.conv = CausalConv3d(
            in_channels, in_channels, kernel_size=3, stride=self.stride, padding=1
        )
...
class Encoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        attn_type="vanilla",
        half_3d=True,
        time_downsample=[True, True, False, False],
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                down.downsample = Downsample3D(
                    block_in, resamp_with_conv, time_downsample[i_level]
                )
                curr_res = curr_res // 2
            self.down.append(down)
...
class Decoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        attn_type="vanilla",
        half_3d=True,
        time_upsample=[False, False, True, True],
        gradient_checkpointing=True,
        **ignorekwargs,
    ):
        super().__init__()
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                up.upsample = Upsample3D(
                    block_in, resamp_with_conv, time_upsample[i_level]
                )
                curr_res = curr_res * 2
            self.up.insert(0, up)  # prepend to get consistent order
        ...
        # end
        self.norm_out = Normalize(block_in)
        self.conv_out = conv_cls(block_in, out_ch, kernel_size=3, stride=1, padding=1)
