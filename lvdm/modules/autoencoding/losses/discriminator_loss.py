...
from lvdm.modules.autoencoding.lpips.model.model import NLayerDiscriminator
...
class GeneralLPIPSWithDiscriminator(nn.Module):
    def __init__(
        ...
        adaptive_disc_weight: bool = True,
    ):
        super().__init__()
        self.dims = dims
        if self.dims > 2:
        ...
        discriminator_config = default(
            ...
            {
                "target": "lvdm.modules.autoencoding.lpips.model.model.NLayerDiscriminator",
                "params": {
    ...
    def log_images(
        ...
        if self.dims > 2 and isinstance(self.discriminator, NLayerDiscriminator):
        ...
        if logits_real.dim() == 5:
            logits_real = rearrange(logits_real, "b c t h w -> (b t) c h w")
            logits_fake = rearrange(logits_fake, "b c t h w -> (b t) c h w")
            inputs = rearrange(inputs, "b c t h w -> (b t) c h w")
            reconstructions = rearrange(reconstructions, "b c t h w -> (b t) c h w")
    ...
    def forward(
        ...
        *,  # added because I changed the order here
        ...
        elif self.rec_loss == "l2":
            rec_loss = torch.nn.functional.mse_loss(
                inputs.contiguous(), reconstructions.contiguous(), reduction="none"
            )
        ...
        if self.dims > 2 and not isinstance(self.discriminator, NLayerDiscriminator):
            reconstructions = rearrange(
                reconstructions, "(b t) c h w -> b c t h w", b=b, t=t
            )
            inputs = rearrange(inputs, "(b t) c h w -> b c t h w", b=b, t=t)
        # now the GAN part
        if optimizer_idx == 0:
            # generator update
            if global_step >= self.discriminator_iter_start or not self.training:
                ...
                if self.training and self.adaptive_disc_weight:
                    d_weight = self.calculate_adaptive_weight(
                        nll_loss, g_loss, last_layer=last_layer
                    )
                elif self.training:
                    d_weight = torch.tensor(self.discriminator_weight).to(
                        rec_loss.device
                    )
                else:
                    d_weight = torch.tensor(1.0).to(rec_loss.device)
            else:
                d_weight = torch.tensor(0.0).to(rec_loss.device)
                g_loss = torch.tensor(0.0, requires_grad=True).to(rec_loss.device)
            ...
            if global_step >= self.discriminator_iter_start or not self.training:
                ...
                d_loss = torch.tensor(0.0, requires_grad=True).to(rec_loss.device)
...
class LPIPSWithDiscriminatorAndDomainConstraint(GeneralLPIPSWithDiscriminator):
    def __init__(
        self,
        disc_start: int,
        logvar_init: float = 0.0,
        ...
        time_n_compress: int = 4,
        rec2d_weight: float = 1.0,
        target_type: str = "slice",
    ):
        super().__init__(
            disc_start,
            logvar_init,
        ...
        self.logvar_2d = nn.Parameter(
            torch.full((), logvar_init), requires_grad=learn_logvar
        )
        self.rec2d_weight = rec2d_weight
        assert target_type in ["random", "slice", "mean"]
        self.target_type = target_type

    def get_trainable_autoencoder_parameters(self) -> Iterator[nn.Parameter]:
        if self.learn_logvar:
            for param in [self.logvar, self.logvar_2d]:
                yield param
        yield from ()
    ...
    def forward(
        ...
        inputs: torch.Tensor,
        reconstructions: torch.Tensor,
        reconstructions_2d: torch.Tensor,
        *,  # added because I changed the order here
        regularization_log: Dict[str, torch.Tensor],
        optimizer_idx: int,
        global_step: int,
        ...
        if self.target_type == "mean":
            targets_2d = torch.cat(
                [
                    inputs[:, :, :1, :, :],
                    rearrange(
                        inputs[:, :, 1:, :, :],
                        "b c (t n) h w -> b c t n h w",
                        n=self.time_n_compress,
                    ).mean(dim=3),
                ],
                dim=2,
            )
        elif self.target_type == "slice":
            targets_2d = inputs[:, :, :: self.time_n_compress, :, :]
        elif self.target_type == "random":
            d = (t - 1) // self.time_n_compress
            indices = torch.cat(
                [
                    torch.tensor([0]),
                    torch.randint(1, self.time_n_compress + 1, (d,))
                    + torch.arange(d) * self.time_n_compress,
                ]
            )
            targets_2d = inputs[:, :, indices, :, :]
        # targets_2d = inputs_t_compressed
        # targets_2d = inputs[:, :, :1, :, :]
        # reconstructions_2d = reconstructions_2d[:, :, :1, :, :]

        inputs, reconstructions, targets_2d, reconstructions_2d = map(
            lambda x: rearrange(x, "b c t h w -> (b t) c h w"),
            (inputs, reconstructions, targets_2d, reconstructions_2d),
        )

        if self.rec_loss == "l1":
            rec2d_loss = torch.abs(
                targets_2d.contiguous() - reconstructions_2d.contiguous()
            )
            rec_loss = torch.abs(inputs.contiguous() - reconstructions.contiguous())
        elif self.rec_loss == "l2":
            rec2d_loss = torch.nn.functional.mse_loss(
                targets_2d.contiguous(),
                reconstructions_2d.contiguous(),
                reduction="none",
            )
            rec_loss = torch.nn.functional.mse_loss(
                inputs.contiguous(), reconstructions.contiguous(), reduction="none"
        ...
        nll_loss, weighted_nll_loss = self.get_nll_loss(rec_loss, rec2d_loss, weights)
        ...
        if optimizer_idx == 0:
            ...
            totoal_loss_log = loss.clone().detach().mean()
            nll_loss_log = nll_loss.detach().mean()
            rec_loss_log = rec_loss.detach().mean()
            rec2d_loss_log = rec2d_loss.detach().mean()
            g_loss_log = g_loss.detach().mean()
            logvar_log = self.logvar.detach()
            logvar_2d_log = self.logvar_2d.detach()
            d_weight_log = d_weight.detach()
            # if (
            #     torch.isnan(totoal_loss_log).any()
            #     or torch.isnan(nll_loss_log).any()
            #     or torch.isnan(rec_loss_log).any()
            #     or torch.isnan(g_loss_log).any()
            #     or torch.isnan(logvar_log).any()
            #     or torch.isnan(d_weight_log).any()
            # ):
            #     print(
            #         f"totoal_loss_log: {totoal_loss_log}, nll_loss_log: {nll_loss_log}, rec_loss_log: {rec_loss_log}, g_loss_log: {g_loss_log}, logvar_log: {logvar_log}, d_weight_log: {d_weight_log}"
            #     )
            #     exit()

            log.update(
                {
                    f"{split}/loss/total": totoal_loss_log,
                    f"{split}/loss/nll": nll_loss_log,
                    f"{split}/loss/rec": rec_loss_log,
                    f"{split}/loss/rec2d": rec2d_loss_log,
                    f"{split}/loss/g": g_loss_log,
                    f"{split}/scalars/logvar": logvar_log,
                    f"{split}/scalars/logvar_2d": logvar_2d_log,
                    f"{split}/scalars/d_weight": d_weight_log,
                }
            )
            ...
            d_loss_log = d_loss.clone().detach().mean()
            logits_real_log = logits_real.detach().mean()
            logits_fake_log = logits_fake.detach().mean()
            # if (
            #     torch.isnan(d_loss_log).any()
            #     or torch.isnan(logits_real_log).any()
            #     or torch.isnan(logits_fake_log).any()
            # ):
            #     print(
            #         f"d_loss_log: {d_loss_log}, logits_real_log: {logits_real_log}, logits_fake_log: {logits_fake_log}"
            #     )
            #     exit()

            log = {
                f"{split}/loss/disc": d_loss_log,
                f"{split}/logits/real": logits_real_log,
                f"{split}/logits/fake": logits_fake_log,
            }
            return d_loss, log
        else:
            raise NotImplementedError(f"Unknown optimizer_idx {optimizer_idx}")
    ...
    def get_nll_loss(
        self,
        rec_loss: torch.Tensor,
        rec2d_loss: torch.Tensor,
        weights: Optional[Union[float, torch.Tensor]] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        nll_loss = rec_loss / torch.exp(self.logvar) + self.logvar
        rec2d_loss = rec2d_loss / torch.exp(self.logvar_2d) + self.logvar_2d
        rec2d_loss = torch.sum(rec2d_loss) / rec2d_loss.shape[0]
        weighted_nll_loss = nll_loss
        if weights is not None:
            weighted_nll_loss = weights * nll_loss
        weighted_nll_loss = torch.sum(weighted_nll_loss) / weighted_nll_loss.shape[0]
        nll_loss = torch.sum(nll_loss) / nll_loss.shape[0]
        weighted_nll_loss = weighted_nll_loss + self.rec2d_weight * rec2d_loss
        nll_loss = nll_loss + self.rec2d_weight * rec2d_loss
        # weighted_nll_loss = (
        #     weighted_nll_loss + torch.sum(rec2d_loss) / rec2d_loss.shape[0]
        # )
        # nll_loss = nll_loss + torch.sum(rec2d_loss) / rec2d_loss.shape[0]

        return nll_loss, weighted_nll_loss


class LPIPSWithDiscriminatorAndEncoderConstraint(GeneralLPIPSWithDiscriminator):
    def __init__(
        self,
        disc_start: int,
        logvar_init: float = 0.0,
...
class LPIPSWithDiscriminatorAndAllConstraint(GeneralLPIPSWithDiscriminator):
    def __init__(
        self,
        disc_start: int,
        logvar_init: float = 0.0,
