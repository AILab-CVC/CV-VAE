...
class JointUCGConditionerForSVD(GeneralConditioner):
    def __init__(
        self,
        emb_models: Union[List, ListConfig],
        joint_ucg_rate: float = 0.0,
        joint_ucg_keys: Optional[Union[List, ListConfig]] = None,
    ):
        torch.nn.Module.__init__(self)
        embedders = []
        self.joint_ucg_rate = joint_ucg_rate
        self.joint_ucg_keys = default(joint_ucg_keys, [])
        for n, embconfig in enumerate(emb_models):
            embedder = instantiate_from_config(embconfig)
            assert isinstance(
                embedder, AbstractEmbModel
    ...
    def forward(
        ...
        if force_zero_embeddings is None:
            force_zero_embeddings = []
        num_video_frames = batch.get("num_video_frames", 1)
        joint_ucg_mask = None
        ...
        for embedder in self.embedders:
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    input_keys = [embedder.input_key]
                    if embedder.legacy_ucg_val is not None:
                        batch = self.possibly_get_ucg_val(embedder, batch)
                    emb_out = embedder(batch[embedder.input_key])
                elif hasattr(embedder, "input_keys"):
                    input_keys = embedder.input_keys
                    emb_out = embedder(*[batch[k] for k in embedder.input_keys])
            assert isinstance(
                emb_out, (torch.Tensor, list, tuple)
            ), f"encoder outputs must be tensors or a sequence, but got {type(emb_out)}"
            if not isinstance(emb_out, (list, tuple)):
                emb_out = [emb_out]
            for emb, input_key in zip(emb_out, input_keys):
                out_key = self.OUTPUT_DIM2KEYS[emb.dim()]
                if embedder.ucg_rate > 0.0 and embedder.legacy_ucg_val is None:
                    if joint_ucg_mask is None:
                        joint_random_p = torch.rand(
                            size=(emb.shape[0] // num_video_frames,), device=emb.device
                        )
                        joint_ucg_mask = (
                            joint_random_p < self.joint_ucg_rate
                        ).repeat_interleave(num_video_frames, dim=0)

                    random_p = torch.rand(
                        size=(emb.shape[0] // num_video_frames,), device=emb.device
                    )
                    ucg_mask = (random_p < embedder.ucg_rate).repeat_interleave(
                        num_video_frames, dim=0
                    )
                    if input_key in self.joint_ucg_keys:
                        ucg_mask = ucg_mask | joint_ucg_mask
                    emb = expand_dims_like(~ucg_mask, emb).to(emb.dtype) * emb
                if (
                    hasattr(embedder, "input_key")
                    and embedder.input_key in force_zero_embeddings
                ):
...
class VideoPredictionEmbedderWithEncoder(AbstractEmbModel):
    def __init__(
        ...
        assert (n_copies == 1) or (n_cond_frames == 1)
    ...
    def forward(
        ...
        if self.is_3d_ae:
            vid = rearrange(vid, "(b t) c h w -> b c t h w", t=self.n_cond_frames)
        ...
        # if self.is_3d_ae:
        #     vid = rearrange(vid, "b c t h w -> b () (t c) h w", t=self.n_cond_frames)
        # else:
...
class EmbedderFromPreprocessedDataset(AbstractEmbModel):
    def __init__(self):
        super().__init__()
...
class FrozenOpenCLIPImagePredictionEmbedder(AbstractEmbModel):
    ...
    def forward(self, vid):
        ...
        vid = rearrange(vid, "b d -> b () d")
        vid = repeat(vid, "b t d -> (b s) t d", s=self.n_copies)
        return vid


class VideoInpaintingMaskEmbedder(AbstractEmbModel):
    def __init__(self, spatial_n_compress=8):
        super().__init__()
        self.spatial_n_compress = spatial_n_compress

    def forward(self, vid):
        # (b t) c h w
        height, width = vid.shape[-2], vid.shape[-1]
        vid = torch.nn.functional.interpolate(
            vid,
            size=(height // self.spatial_n_compress, width // self.spatial_n_compress),
        )
