...
from lvdm.inference.helpers import Img2ImgDiscretizationWrapper, do_img2img, do_sample
from lvdm.modules.diffusionmodules.sampling import (
    DPMPP2MSampler,
...
from lvdm.util import load_model_from_config
...
def get_guider_config(params: SamplingParams):
    if params.guider == Guider.IDENTITY:
        guider_config = {
            "target": "lvdm.modules.diffusionmodules.guiders.IdentityGuider"
        }
        ...
        if thresholder == Thresholder.NONE:
            dyn_thresh_config = {
                "target": "lvdm.modules.diffusionmodules.sampling_utils.NoDynamicThresholding"
            }
        else:
            raise NotImplementedError

        guider_config = {
            "target": "lvdm.modules.diffusionmodules.guiders.VanillaCFG",
...
def get_discretization_config(params: SamplingParams):
    if params.discretization == Discretization.LEGACY_DDPM:
        discretization_config = {
            "target": "lvdm.modules.diffusionmodules.discretizer.LegacyDDPMDiscretization",
        }
    elif params.discretization == Discretization.EDM:
        discretization_config = {
            "target": "lvdm.modules.diffusionmodules.discretizer.EDMDiscretization",
            "params": {
