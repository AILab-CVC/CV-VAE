...
from lvdm.util import exists, instantiate_from_config, isheatmap
...
def init_wandb(save_dir, opt, config, group_name, name_str):
    ...
    print(
        f"wandb init with: group_name: {group_name}, name_str: {name_str}, projectname: {opt.projectname}"
    )
    if opt.debug:
        ...
        wandb.init(
            ...
            mode="offline",
            settings=wandb.Settings(code_dir="./lvdm"),
            group=group_name,
            name=name_str,
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "batch_size" in config.data.params:
            ...
            bs, base_lr = (
                config.data.params.train.dataloader.batch_size,
        ...
        if opt.train:
            try:
                if (
                    "strategy" in trainer_opt
                    and "deepspeed" in trainer_opt["strategy"]
                    and "precision" in trainer_opt
                    and "16" in str(trainer_opt["precision"])
                ):
                    print("Using mixed-precision autocast.")
